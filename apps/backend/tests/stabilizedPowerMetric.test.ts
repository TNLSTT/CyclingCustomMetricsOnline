...
import { stabilizedPowerMetric } from '../src/metrics/stabilizedPower.js';
...
describe('stabilizedPowerMetric', () => {
  it('computes stabilized power statistics for variable efforts', async () => {
    ...
    const result = await stabilizedPowerMetric.compute(samples, { activity });
    const summary = result.summary as Record<string, unknown>;

    const stabilizedPower = summary.stabilized_power_w as number;
    ...
    expect(typeof stabilizedPower).toBe('number');
    expect(typeof averagePower).toBe('number');
    expect(stabilizedPower).toBeGreaterThan(averagePower);
  ...
  it('yields null stabilized power when insufficient samples are present', async () => {
    ...
    expect(summary.stabilized_power_w).toBeNull();
