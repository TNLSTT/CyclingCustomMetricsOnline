...
File: `apps/backend/src/metrics/stabilizedPower.ts`

- Uses 30-second rolling averages (sample-rate aware) to compute **stabilized power**, **average power**, and the **variability index**.
- Tracks coasting prevalence (≤ 5 W), counts of valid/total power samples, and window metadata to aid data-quality audits.
- Emits a rolling power series so the frontend can visualize stabilized power smoothing alongside instantaneous power.
...
- Stabilized power for the opening and closing thirds, expressed as a percentage of FTP.
...
- Subtract **0.5 points** for every percentage-point drop in stabilized power %FTP from early to late thirds.
